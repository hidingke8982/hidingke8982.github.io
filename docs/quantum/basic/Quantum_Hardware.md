...
## 양자 컴퓨터의 동작 방식
1. 양자 회로: 양자 회로가 제어 시스템에 입력됨.
2. 펄스 시퀀스: 게이트는 마이크로파 펄스 등의 물리적 신호로 변환되어 큐비트에 인가됨.
3. 상태 진화: 큐비트 상태는 유니타리 연산에 의해 연속적으로 회전하며 진화함.
4. 측정: 최종 상태를 관측하면 파동 함수 붕괴가 일어나며, 이 결과를 통계적으로 집계함.
...
### 결잃음(Decoherence)
큐비트가 주변 환경(열, 전자기 노이즈)과 상호작용하여 양자 상태를 잃고 고전적인 상태로 돌아가려는 현상이다. 양자 컴퓨터는 계산 시간은 반드시 위상 감쇠 시간($T_2$)보다 짧아야 정확한 결과를 얻을 수 있다. 이것이 양자 컴퓨터가 극저온 환경 등 극한의 환경에서 작동해야 하는 이유다.
- 이완 시간 ($T_1$): 큐비트가 에너지를 잃고 초기 상태($\vert 0 \rangle$ 또는 $\vert 1 \rangle$) 중 하나로 돌아가는 시간.
- 위상 감쇠 시간 ($T_2$): 큐비트가 중첩 상태를 잃고 고전적으로 붕괴되는 시간. $T_2$는 보통 $T_1$보다 짧다.

결잃음은 양자 컴퓨터가 "일정 시간 내에 오류 없이 계산을 완료할 수 있는가"를 결정하는 근본적인 장벽이며, 이 문제 해결은 양자 우위(Quantum Advantage)를 달성하기 위한 가장 큰 엔지니어링 과제다.
